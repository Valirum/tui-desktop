...
def move_fav_item(workspace_win, dx=0, dy=0):
    global hovered_desktop_item, desktop_items, config
    if desktop_mode != "favorite" or hovered_desktop_item < 0:
        return
    if hovered_desktop_item >= len(desktop_items):
        return

    item = desktop_items[hovered_desktop_item]
    fav_index = item.get('fav_index')
    if fav_index is None:
        return

    fav = config["paths"]["favorite"]
    old_x = fav[fav_index].get("x", 0)
    old_y = fav[fav_index].get("y", 0)
    ...
    # Проверяем коллизию (опционально: можно разрешить наложение)
    for i, other in enumerate(fav):
        if i != fav_index and other.get("x") == new_x and other.get("y") == new_y:
            # Меняем местами
            fav[i]["x"], fav[i]["y"] = old_x, old_y
            break

    # Обновляем позицию
    fav[fav_index]["x"] = new_x
    fav[fav_index]["y"] = new_y

    # Сохраняем
    with open("config.json", "w") as f:
        json.dump(config, f, indent=4)

    # Пересканируем и перерисуем
    scan_desktop_items(workspace_win)
    # Пытаемся сохранить выделение по fav_index
    for i, it in enumerate(desktop_items):
        if it.get('fav_index') == fav_index:
            hovered_desktop_item = i
            break

def is_in_favorite(path):
    fav = config["paths"].get("favorite", {"files": [], "folders": []})
    abs_path = os.path.abspath(path)
    return abs_path in [os.path.abspath(p) for p in fav["files"] + fav["folders"]]
...
def scan_desktop_items(workspace_win):
    ...
    if desktop_mode == "favorite":
                fav = config["paths"].get("favorite", [])
                # Создаём сетку позиций → индекс
                pos_to_item = {}
                for i, item in enumerate(fav):
                    x = item.get("x", i)  # fallback: линейный порядок
                    y = item.get("y", 0)
                    pos_to_item[(y, x)] = i
            
                # Определяем размер сетки
                if fav:
                    max_x = max(item.get("x", i) for i, item in enumerate(fav))
                    max_y = max(item.get("y", 0) for item in fav)
                else:
                    max_x = max_y = 0
            
                # Заполняем desktop_items в порядке сетки (сверху вниз, слева направо)
                desktop_items = []
                icon_cols = workspace_win.getmaxyx()[1] // 9 or 1
                for y in range(max_y + 1):
                    for x in range(icon_cols):
                        if (y, x) in pos_to_item:
                            idx = pos_to_item[(y, x)]
                            raw = fav[idx]
                            path = os.path.expanduser(raw["path"])
                            ...
                            desktop_items.append({
                                ...
                                'fav_index': idx  # ← чтобы знать, какой элемент в избранном
                            })
    else:
        try:
            items = []
    ...
    icon_cols = workspace_win.getmaxyx()[1] // 9
    for i, _ in enumerate(desktop_items):
        grid_y = i // icon_cols
        grid_x = i % icon_cols
        desktop_grid[(grid_y, grid_x)] = i
        ...
        for (grid_y, grid_x), item_index in desktop_grid.items():
            if item_index < len(desktop_items):
                ...
                if screen_y + 7 < max_y and screen_x + 9 < max_x:
                    ...
                    elif item.get('in_favorite', False):
...
def handle_main_input(ch, workspace_win):
    ...
    if ch in (curses.KEY_ENTER, 10, 13):
        ...
        hovered_desktop_item = max(0, hovered_desktop_item-(workspace_win.getmaxyx()[1]//9))
    elif ch == curses.KEY_LEFT:
        hovered_desktop_item = max(0, hovered_desktop_item-1)
    elif ch == curses.KEY_RIGHT:
        hovered_desktop_item = min(desktop_items.__len__()-1, hovered_desktop_item+1)
    elif ch == curses.KEY_DOWN:
        hovered_desktop_item = min(desktop_items.__len__()-1, hovered_desktop_item+(workspace_win.getmaxyx()[1]//9))
        ...
        if hovered_desktop_item >= 0 and desktop_mode == "favorite":
            ...
            full_path = item['path']
            if item['type'] == 'folder':
                if full_path in fav["folders"]:
                    fav["folders"].remove(full_path)
            else:
                if full_path in fav["files"]:
                    fav["files"].remove(full_path)
            with open("config.json", "w") as f:
                json.dump(config, f, indent=4)
            scan_desktop_items(workspace_win)
            #hovered_desktop_item -= 1
        ...
        move_fav_item(workspace_win, dx=-1)
    elif ch == curses.KEY_SRIGHT:     # Shift+Right
        move_fav_item(workspace_win, dx=+1)
    elif ch == 337:                   # Shift+Up (в curses часто 337)
        move_fav_item(workspace_win, dy=-1)
    elif ch == 336:                   # Shift+Down
        move_fav_item(workspace_win, dy=+1)
    else:
        pass
...
def show_new_tab_dialog(workspace_win, tab_win):
    ...
    while True:
        ...
        elif 66 <= ch <= 126:  # Печатаемые символы
            input_buffer += chr(ch)
            input_win.addch(ch)
        else:
            pass
